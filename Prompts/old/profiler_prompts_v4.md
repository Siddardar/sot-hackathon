...
`{CATEGORY_BLOCK}` is `render_categories_for_prompt()` from `taxonomy_v3.py`. When the export has no timestamps, call it with `include_tiers` excluding `"M"` so the model is not tempted to invent them.
