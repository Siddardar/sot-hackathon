...
TASK
Extract direct, non-sensitive inferences: facts that are not stated outright but follow from one cue in one message.

A finding belongs in Tier B when a single quoted cue is enough to support the inference. The cue is explicit, but the conclusion is not.
...
EXAMPLES
- "I need to catch the U6 after class" -> possible student status or location context, depending on the cue and mode.
...
REASONING
Use one or two sentences naming the single inference step from cue to conclusion.
